...
function Menu() {
  ...
  useEffect(() => {
    if (!tableNumber || isNaN(tableNumber) || Number(tableNumber) < 1) {
      navigate('/scan-qr', { state: { message: 'Invalid table number. Please scan a valid QR code.' } });
      return;
    }

    // Validate session or create a new one
    const validateOrCreateSession = async () => {
      try {
        const token = localStorage.getItem('sessionToken');
        if (token) {
          const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/sessions/validate/${token}`);
          if (response.data.tableNumber !== Number(tableNumber)) {
            localStorage.removeItem('sessionToken');
            throw new Error('Session table mismatch');
          }
          if (response.data.orderId) {
            navigate(`/summary?order=${response.data.orderId}`);
            return;
          }
        } else {
          const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/sessions`, { tableNumber });
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { tableNumber, items: cart.map(item => ({ itemId: item.itemId, quantity: item.quantity })) },
    ...
    <div className="min-h-screen bg-orange-50 pb-32">
      <header className="sticky top-0 shadow-lg z-50 border-b-2 border-amber-900" style={{ backgroundColor: '#92400e', color: '#ffffff' }}>
        <div className="container mx-auto p-2 sm:p-4 flex justify-between items-center">
          ...
          <span className="text-xs sm:text-sm font-medium flex items-center px-2 py-1 rounded-full" style={{ backgroundColor: '#b45309', color: '#ffffff' }}>
            ...
            {categories.map(category => (
              <button
                ...
                className={`flex items-center px-3 sm:px-4 py-1 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${
                  selectedCategory === category.name ? 'shadow-md hover:bg-amber-700' : 'hover:bg-amber-300'
                }`}
                style={{
      ...
      <div className="container mx-auto p-2 sm:p-4">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-3">
          {filteredItems.length === 0 ? (
            <p className="text-center col-span-full text-gray-600 text-sm">No items available.</p>
          ) : (
                ...
                <div className="flex-1">
                  ...
                  <button
                    className={`mt-2 sm:mt-3 w-full text-white px-2 sm:px-4 py-1 sm:py-2 rounded-lg transition-colors flex items-center justify-center text-xs sm:text-sm ${
                      item.isAvailable ? 'hover:bg-green-600 focus:ring-2 focus:ring-green-400' : 'bg-red-500 hover:bg-red-600 cursor-not-allowed'
              ...
              <div className="mt-2 sm:mt-4">
                {cart.length === 0 ? (
                  ...
                  <div className="max-h-36 overflow-y-auto space-y-2">
                    {cart.map(item => (
                      <div key={item.itemId} className="flex items-center py-1 sm:py-2 border-b">
                        <img
                          src={item.image}
                          alt={item.name}
